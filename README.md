...
- `src/worker.ts` serves built assets, supports temporary admin login from `ADMIN_EMAIL` / required `ADMIN_PASSWORD`, returns a minimal `/api/summary`, and keeps clear placeholders for data-writing APIs and `/ws` until the backend migration is complete.
